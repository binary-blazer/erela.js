...
export class Player {
   ...
   * @returns 
   */
  public async setBassboostGain(gain = 0): Promise<boolean> {
    if(this.node.info && !this.node.info?.filters?.includes("bassboost")) throw new Error("Node#Info#filters does not include the 'bassboost' Filter (Node has it not enable)")
    // reset nightcore / vaporwave filter if enabled
    if(this.filters.nightcore || this.filters.vaporwave) { 
      this.filterData.timescale.pitch = 1;
      this.filterData.timescale.speed = 1;
   ...
   * @returns 
   */
  public async setBassboostThreshold(threshold = 0): Promise<boolean> {
    if(this.node.info && !this.node.info?.filters?.includes("bassboost")) throw new Error("Node#Info#filters does not include the 'bassboost' Filter (Node has it not enable)")
    // reset nightcore / vaporwave filter if enabled
    if(this.filters.nightcore || this.filters.vaporwave) { 
      this.filterData.timescale.pitch = 1;
   ...
   * @returns
   */
  public async toggleBassboost(gain = 0.5, threshold = 0.5): Promise<boolean> {
      if(this.node.info && !this.node.info?.filters?.includes("bassboost")) throw new Error("Node#Info#filters does not include the 'bassboost' Filter (Node has it not enable)")
